...
uint8_t I2C_ReadContinoues(i2c_register_t *Base, uint8_t Address, uint8_t *Data, uint32_t len){

	uint32_t dataToBeRead = len;
	uint32_t loop = 0;
	Base->I2C_CR1 |= 1 << 10;

	while(dataToBeRead > 2) {
		while(!(Base->I2C_SR1 & (1 << 6) ));
		Data[loop++] = Base->I2C_DR;
		dataToBeRead--;
	}
...
uint8_t I2C_Start(i2c_register_t *Base, uint8_t slaveAddr, uint8_t mode) {
